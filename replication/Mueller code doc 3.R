...
#setwd("/Users/smueller/Google Drive/papers/Müller_Regan Paper/replication_materials_ips")
...
## When I started running the code to compile the graphs, the legends on the x axis
# were too big and the writing was overlapping. So I changed the size in
# the code above to "10" from "13", which acts as a template for all the graphs.
...
ggsave("fig_02.pdf",
       width = 9, height = 7.5)
## Here the Y axis legend was way too big and all the dates were overlaping and 
# unreadable so I went back to the template and changed that. 

## Linear regression models ----


dat_reg <- dat_electionstudies
#### Why is the dat_electionstudies being renamed? 
#### Before we go on, I just want to look at dat_reg a little.
summary(dat_reg)
colnames(dat_reg)
head(dat_reg)
tail(dat_reg)
is.na(dat_reg)
## We can see the years in the column "year" range from 2002 to 2020.
## Also there are quite a few NAs
...
## Error message: Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : 
# contrasts can be applied only to factors with 2 or more levels
## This error message is related to how R handles categorical variables when 
# fitting a linear regression model using the lm() function.
## It looks as though the data contains a categorical variable, probably
# the variable year, which only has one level in the subset of data,
# which won´t fit the model. The filter() function is used to subset
# the original data to include only observations from the year 2020.
# However, if the original data only contained observations from 2020,
# the year variable would have only one level, which would result in this
# error.

# When you fit a linear regression model with a categorical variable in R,
# the categorical variable is automatically converted into a set of dummy
# variables (also known as indicator variables) which represent the 
# different levels of the variable. Each level of the categorical variable
# is represented by a binary variable (0 or 1), and one level is chosen as
# the reference level (usually the first level in alphabetical order).

# The error message is telling you that there is only one level of the
# categorical variable in the subset of data you are trying to fit the
# updated model to. Since there is only one level, there is no variation 
# in the values of the dummy variables, and therefore the model cannot be 
# fitted.To fix this error, we have to check the original data to make 
# sure that it contains observations from more than one year. 
# If there are observations from only one year, we need to revise the
# analysis to include data from multiple years, or remove the year 
# variable from your analysis. If there are observations from multiple 
# years, you may want to check the syntax of your filter() function to 
# make sure that it is correctly subsetting the data based on the year variable.
## However, the dates in the year column range from 2002 to 2020. So there could be missing values.
sum(is.na(dat_reg$year))
# No, no missing values 

# There are definitely missing values in other columns in some rows though. Will try to remove them.
dat_reg <- dat_reg[complete.cases(dat_reg), ]
# This is not helping either as I tried the regression again and got the same error message.
levels(dat_reg$year)
## If the output shows only one level, that confirms that year is not a factor variable in the dataset, 
## and yet the same function worked for the other years.
## Will try to convert year to a factor variable using the as.factor() function. 
dat_reg$year <- as.factor(dat_reg$year)
# Still not working. Will check the source data.
## What is going on with the 2020 values?
dat_reg_2020 <- dat_reg[dat_reg$year == 2020, ]
dat_reg_2020
# Count the number of missing values in each column 
colSums(is.na(dat_reg_2020)) 
#No missing values.
head(dat_reg_2020)
tail(dat_reg_2020)
dat_reg_no_na <- na.omit(dat_reg)
str(dat_reg)
dat_reg$year <- as.numeric(as.character(dat_reg$year))
lm_lr_20 <- update(lm_lr_02, data = filter(dat_reg_no_na, year == 2020))
## Doesn´t even work when nas are removed.
## How did this work for Mueller????
unique(dat_reg$year)
str(dat_reg$year)
dat_reg$year <- as.numeric(as.character(dat_reg$year)) 
## I am still getting the error message lm_lr_20 <- update(lm_lr_02, data = filter(dat_reg, year == 2020))
##Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : 
##  contrasts can be applied only to factors with 2 or more levels
dat_reg$year <- as.factor(dat_reg$year) 
# Convert the year variable to a factor 
lm_lr_02 <- lm(left_right_self ~ income_harmonised + age_cat + gender + urban + university_degree, weight = weights, data = dat_reg) lm_lr_20 <- update(lm_lr_02, data = subset(dat_reg, year == 2020)) 
# Fit the model for 2020 with the updated data frame 
lm_lr_20 <- update(lm_lr_02, data = filter(dat_reg, year == 2020))
dat_reg$year <- as.numeric(as.character(dat_reg$year)) # Convert year variable to numeric lm_lr_02 <- lm(left_right_self ~ income_harmonised + age_cat + gender + urban + university_degree, weight = weights, data = dat_reg) # Fit the model with the updated year variable lm_lr_20 <- update(lm_lr_02, data = filter(dat_reg, year == 2020)) 
summary(dat_reg_2020)
## OMG, the whole thing is zeros and missing values.
...
## I´m getting the error message "Error in "list" %in% class(l)[1] : 
## object 'lm_lr_20' not found" because the last lm didn´t work.
## So will remove it. 
screenreg(list(
  lm_lr_02,
  lm_lr_07,
